"""
Value iteration on null maze gridworld.
...
def main() -> None:
    ...
    ax.get_figure().savefig(os.path.join(OUTPUT_DIR, "shaping_maze_gridworld.png"), dpi=150)
    plt.close(ax.get_figure())

    # Solve the MDP with value iteration.
    V, policy, history = value_iteration(env, gamma=0.9999, theta=1e-8)
    ...
    ax.get_figure().savefig(os.path.join(OUTPUT_DIR, "value_function_shaping_maze.png"), dpi=150)
    plt.close(ax.get_figure())

    ax = plot_policy(env, policy, title="Value iteration: optimal policy")
    ax.get_figure().savefig(os.path.join(OUTPUT_DIR, "optimal_policy_shaping_maze.png"), dpi=150)
    plt.close(ax.get_figure())
    ...
    save_path = os.path.join(OUTPUT_DIR, "value_iteration_shaping_maze.gif")
