"""
Runs q-learning on the maze gridworld with Q initialised to zero. 
...
def main() -> None:
    ...
    plot_learning_curve(
        episode_returns,
        save_path=os.path.join(OUTPUT_DIR, "null_maze_q_learning_curve_.png"),
        title="Q-learning: episode return on null maze GridWorld",
    )

    ax = plot_policy(env, policy, title="Q-learning: greedy policy")
    ax.get_figure().savefig(os.path.join(OUTPUT_DIR, "null_maze_q_learning_policy.png"), dpi=150)
    plt.close(ax.get_figure())

    # Also save a Q-value grid (action-values in each state).
    ax = plot_q_values(env, Q, title="Q-learning: Q(s, a)")
    ax.get_figure().savefig(os.path.join(OUTPUT_DIR, "null_maze_q_learning_q_values.png"), dpi=150)
    plt.close(ax.get_figure())
    ...
    save_path = os.path.join(OUTPUT_DIR, "null_maze_q_learning_agent.gif")
