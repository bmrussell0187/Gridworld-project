"""Vary ONE parameter of the mine gridworld experiment.
...
saves the results to outputs/ as
    ...
    mine_sweep_<EXPERIMENT>.json         the settings + one entry per seeded run
    mine_sweep_<EXPERIMENT>_arrays.npz   the big per-episode arrays
...
#             "env"   if it is an argument of make_deep_mining_gridworld.
...
ENV_SETTINGS = {    # arguments of make_deep_mining_gridworld
    "step_reward": -0.01,
...
def run_one_agent(env_settings, agent_settings, seed):
    ...
    env = make_deep_mining_gridworld(seed=seed, **env_settings)
...
arrays_filename = f"mine_sweep_{EXPERIMENT}_arrays.npz"
json_path = os.path.join(OUTPUT_DIR, f"mine_sweep_{EXPERIMENT}.json")
...
probe_env = make_deep_mining_gridworld(seed=0, **ENV_SETTINGS)
results = {
